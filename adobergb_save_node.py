...
try:
    ...
    tifffile = None
    print(f"\033[92m[adobeRGB-save]\033[0m tifffile not available. 16-bit TIFF disabled.")

try:
    import imagecodecs
except ImportError:
    imagecodecs = None
    print(f"\033[92m[adobeRGB-save]\033[0m imagecodecs not available. 16-bit PNG disabled.")
...
def _build_adobe_rgb_icc():
    ...
    tag_TRC = _build_icc_curv_tag(_ADOBE_GAMMA)   # shared by rTRC, gTRC, bTRC

    # desc tag (type 'desc') — full ICC v2.1 structure
    ...
    # Unicode localizable string: language code (4) + count (4)
    tag_desc += b'\x00' * 4                        # Unicode language code
    tag_desc += struct.pack('>I', 0)               # Unicode description count = 0
    # ScriptCode section: code (2) + count (1) + fixed string (67)
    tag_desc += b'\x00' * 2                        # ScriptCode code
    tag_desc += b'\x00'                            # ScriptCode count = 0
    tag_desc += b'\x00' * 67                       # ScriptCode string (fixed 67 bytes)
    if len(tag_desc) % 4 != 0:
    ...
    # Tag table: 9 tags (rTRC/gTRC/bTRC share one data block)
    # Data blocks stored in tag_data; rTRC/gTRC/bTRC point to same offset.
    tag_count = 9
    header_size = 128
    tag_table_size = 4 + tag_count * 12  # count(4) + 9 entries(9*12)

    # Layout tag data blocks (unique only — TRC stored once)
    data_offset = header_size + tag_table_size
    tag_data = b''

    off_desc = data_offset + len(tag_data); tag_data += tag_desc
    off_wtpt = data_offset + len(tag_data); tag_data += tag_wtpt
    off_rXYZ = data_offset + len(tag_data); tag_data += tag_rXYZ
    off_gXYZ = data_offset + len(tag_data); tag_data += tag_gXYZ
    off_bXYZ = data_offset + len(tag_data); tag_data += tag_bXYZ
    off_TRC  = data_offset + len(tag_data); tag_data += tag_TRC   # one copy
    off_cprt = data_offset + len(tag_data); tag_data += tag_cprt

    # Build tag table entries — sorted ascending by 4-byte signature (ICC requirement)
    # bTRC(0x62545243) < bXYZ(0x6258595A) < cprt(0x63707274) < desc(0x64657363)
    # < gTRC(0x67545243) < gXYZ(0x6758595A) < rTRC(0x72545243) < rXYZ(0x7258595A)
    # < wtpt(0x77747074)
    tag_entries = [
        struct.pack('>4sII', b'bTRC', off_TRC,  len(tag_TRC)),   # shared
        struct.pack('>4sII', b'bXYZ', off_bXYZ, len(tag_bXYZ)),
        struct.pack('>4sII', b'cprt', off_cprt, len(tag_cprt)),
        struct.pack('>4sII', b'desc', off_desc, len(tag_desc)),
        struct.pack('>4sII', b'gTRC', off_TRC,  len(tag_TRC)),   # shared
        struct.pack('>4sII', b'gXYZ', off_gXYZ, len(tag_gXYZ)),
        struct.pack('>4sII', b'rTRC', off_TRC,  len(tag_TRC)),   # shared
        struct.pack('>4sII', b'rXYZ', off_rXYZ, len(tag_rXYZ)),
        struct.pack('>4sII', b'wtpt', off_wtpt, len(tag_wtpt)),
    ]
...
class SaveImageExtendedAdobeRGB:
    ...
    print(f"\033[92m[adobeRGB-save] version: {version}\033[0m")
    if jxl_supported:
...
All filename/folder templating features from Save Image Extended are included."""
    ...
    def get_latest_counter(self, folder_path, filename, counter_digits=4, counter_position='last', output_ext='.png'):
        ...
        if not os.path.exists(folder_path):
            print(f"[adobeRGB-save] Folder {folder_path} does not exist, starting counter at 1.")
            return counter
        try:
            ...
            if files:
                if counter_position not in self.counter_positions:
                    counter_position = self.counter_position

                if not filename:
                    # Counter-only filenames like 0001.png — match by length
                    counters = [int(f[:counter_digits]) if (f[:counter_digits].isdecimal() and len(f) == (counter_digits + extLen)) else 0 for f in files]
                else:
                    if counter_position == 'last':
                        counters = [int(f[-(extLen + counter_digits):-extLen]) if f[-(extLen + counter_digits):-extLen].isdecimal() else 0 for f in files if f.startswith(filename)]
                    else:
                        counters = [int(f[:counter_digits]) if f[:counter_digits].isdecimal() else 0 for f in files if f[counter_digits + 1:].startswith(filename)]

                if counters:
                    counter = max(counters) + 1

        except Exception as e:
            print(f"[adobeRGB-save] Error finding latest counter: {e}")
        return counter
    ...
    # -------------------------------------------------------------------
    # Filename / foldername generation
    # -------------------------------------------------------------------
                        ...
                        if '' not in splitKey:
                            if splitKey[0].isdecimal():
                                ...
                                if node not in prompt:
                                    print(f"[adobeRGB-save] node #{node} not found, searching all")
    ...
    # -------------------------------------------------------------------
    # 8-bit image writer (Pillow) with ICC profile support
    # -------------------------------------------------------------------
    ...
    def writeImage(self, image_path, img, prompt, save_metadata=True, extra_pnginfo=None, quality=90, icc_profile=None):
        ...
        if output_ext in ['.avif', '.webp', '.jxl']:
        ...
        elif output_ext in ['.j2k', '.jp2', '.jpc', '.jpf', '.jpx', '.j2c']:
            if save_metadata:
                kwargs['exif'] = self.genMetadataEXIF(img, prompt, extra_pnginfo)
            if quality < 100:
            ...
            if save_metadata:
                kwargs['exif'] = self.genMetadataEXIF(img, prompt, extra_pnginfo)
            # Note: Pillow's TIFF writer does not accept 'optimize' kwarg
            if icc_profile is not None:
                kwargs['icc_profile'] = icc_profile
    ...
    def save_images(self,
        ...
        if bit_depth == '16' and not use_16bit:
            print(f"[adobeRGB-save] 16-bit not supported for {output_ext}, falling back to 8-bit.")

        # Additional guard: 16-bit PNG requires imagecodecs, 16-bit TIFF requires tifffile
        if use_16bit and output_ext == '.png' and imagecodecs is None:
            print(f"[adobeRGB-save] imagecodecs not available, falling back to 8-bit PNG.")
            use_16bit = False
        if use_16bit and output_ext == '.tiff' and tifffile is None:
            print(f"[adobeRGB-save] tifffile not available, falling back to 8-bit TIFF.")
            use_16bit = False
