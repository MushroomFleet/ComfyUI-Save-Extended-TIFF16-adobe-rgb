...
def _build_adobe_rgb_icc():
    ...
    # Minimal Unicode localizable string area: language code (4) + count (4) = 8 bytes of zeros
    tag_desc += b'\x00' * 8
    if len(tag_desc) % 4 != 0:
...
All filename/folder templating features from Save Image Extended are included."""
    ...
    def get_latest_counter(self, folder_path, filename, counter_digits=4, counter_position='last', output_ext='.png'):
        ...
        try:
            ...
            print(f"[save_image_adobeRGB] Error finding latest counter: {e}")
        return counter
    ...
    def save_images(self,
        ...
        if use_16bit and output_ext == '.png' and imagecodecs is None:
            print(f"[save_image_adobeRGB] imagecodecs not available, falling back to 8-bit PNG.")
            use_16bit = False
        if use_16bit and output_ext == '.tiff' and tifffile is None:
            print(f"[save_image_adobeRGB] tifffile not available, falling back to 8-bit TIFF.")
            use_16bit = False
